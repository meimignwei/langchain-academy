...
Return your response as a JSON object: {{"subjects": ["topic1", "topic2", "topic3"]}}"""
joke_prompt = """Generate a joke about {subject}. Return your response as a JSON object: {{"joke": "your joke here"}}"""
best_joke_prompt = """Below are a bunch of jokes about {topic}. Select the best one! Return the ID of the best one, starting 0 as the ID for the first joke. Return your response as a JSON object: {{"id": 0}}
...
def best_joke(state: OverallState):
    ...
    response = model.invoke(prompt)
    result = json.loads(response.content.strip().removeprefix("```json").removesuffix("```").strip())
    return {"best_selected_joke": state["jokes"][result["id"]]}
